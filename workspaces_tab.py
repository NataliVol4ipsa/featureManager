    ...
    def _action_push(self):
        ...
        if not ok:
            if workspace is not None:
                self.errors.add(entries)
            return
        # Non-ignored repos are pushed; each is compared to the feature branch
        # configured for it in the workspace, not to the other repos.
        active = [e for e in entries if not e["ignoreGit"]]
        repos = [(e["name"], e["path"]) for e in active]
        expected_branches = {e["name"]: e["branch"] for e in active}
        self.push_all(repos, expected_branches=expected_branches)
